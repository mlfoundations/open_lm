...
def make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, epoch_num=2, ratio=0.1, name_suffix="", title=True):
    """
    ...
    if "1b" in base_dir_1:
        filtered_df = data_df[
            ((abs(data_df['datacompdr1b_caption']/data_df['llava'] - 4) < 0.1) |
            ((data_df['datacompdr1b_caption'] == 0) & (data_df['llava'] == 0))) &
    ...
    # Plotting
    ax2 = ax.twinx()
    sorted_df = filtered_df.sort_values(by='bucket')
    
    # Vision score - using a more vibrant green
    vision_color = '#7CAB66'  # Jade Green
    
    # Split the data into two parts: all points except the last, and the last point
    vision_data = sorted_df['x1']
    vision_x = sorted_df['bucket']
    
    ax.plot(vision_x[:-1], vision_data[:-1], 
            color=vision_color, marker='o', linestyle='-', 
            label=f'Vision {vision_eval}', linewidth=2, markersize=8)
    
    # Plot the last point with a hollow marker and dashed line
    ax.plot(vision_x[-2:], vision_data[-2:], 
            color=vision_color, marker='o', linestyle='--', 
            linewidth=2, markersize=8, markerfacecolor='none', markeredgewidth=2)
    
    # Text score - using a more vibrant blue
    text_color = '#4169E1'  # Royal Blue
    
    # Split the data for text score as well
    text_data = sorted_df['x2']
    
    ax2.plot(vision_x[:-1], text_data[:-1], 
             color=text_color, marker='o', linestyle='-', 
             label=f'Text {text_eval}', linewidth=2, markersize=8)
    
    # Plot the last point for text score
    ax2.plot(vision_x[-2:], text_data[-2:], 
             color=text_color, marker='o', linestyle='--', 
             linewidth=2, markersize=8, markerfacecolor='none', markeredgewidth=2)
    
    ax.set_xlabel('Text pretraining completion ratio', fontsize=14, fontweight='bold')
    ax.set_ylabel(f'Vision {vision_eval}', color=vision_color, fontsize=14, fontweight='bold')
    ax2.set_ylabel(f"Text {text_eval}", color=text_color, fontsize=14, fontweight='bold')

    if title:
        ax.set_title(f'Vision: {vision_eval}', fontsize=14, fontweight='bold')
    ...
    # Set y-axis colors
    ax.spines['left'].set_color(vision_color)
    ax.tick_params(axis='y', colors=vision_color)
    ax2.spines['right'].set_color(text_color)
    ax2.tick_params(axis='y', colors=text_color)

   
def create_grid_plot(base_dir_1, base_dir_2, key_text, epoch_num=2, ratio=0.1, name_suffix="", output_dir=""):
    ...
    for i, (key_list_vision, vision_name) in enumerate(vision_evals.items()):
        ...
        make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, epoch_num, ratio, name_suffix)
    
    plt.tight_layout()
    ratio = str(ratio).replace(".", "p")
    if output_dir[-1] != "/":
        output_dir += "/"
    plt.savefig(f'{output_dir}text_pretraining_vs_vision_text{text_evals[key_list_text]}_grid_epoch{epoch_num}_ratio{ratio}{name_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
def create_single_plot(base_dir_1, base_dir_2, key_text, key_vision, epoch_num=2, ratio=0.1, name_suffix="", output_dir=""):
    
    key_list_text = key_text
    key_list_vision = key_vision
    
    fig, ax = plt.subplots(figsize=(6, 3))
    plt.style.use('seaborn-v0_8-pastel')
    
    make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, epoch_num, ratio, name_suffix, title=False)
    
    plt.tight_layout()
    ratio = str(ratio).replace(".", "p")
    if output_dir[-1] != "/":
        output_dir += "/"
    plt.savefig(f'{output_dir}text_pretraining_vs_vision{vision_evals[key_list_vision]}_text{text_evals[key_list_text]}_single_epoch{epoch_num}_ratio{ratio}{name_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epoch_num", type=int, default=4, help="Epoch number to plot.")
    ...
    if args.vision_key == "all":
        create_grid_plot(base_dir_1, base_dir_2, args.text_key, args.epoch_num, args.ratio, args.name_suffix, args.output_dir)
    else:
        create_single_plot(base_dir_1, base_dir_2, args.text_key, args.vision_key, args.epoch_num, args.ratio, args.name_suffix, args.output_dir)
