...
def make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, epoch_num=2, bucket=5, name_suffix="", title=True):
    ...
    # Plotting
    ax2 = ax.twinx()
    sorted_df = filtered_df.sort_values(by='pretraining_image_ratio')
    
    # Vision score - using a more vibrant green
    vision_color = '#7CAB66'  # Jade Green 00A86B
    ax.plot(sorted_df['pretraining_image_ratio'].clip(1e-3), sorted_df['x1'], 
    ...
    ax2.plot(sorted_df['pretraining_image_ratio'].clip(1e-3), sorted_df['x2'], 
    ...
    ax.set_xticks(list(sorted_df['pretraining_image_ratio'].clip(1e-3).unique()))
...
def create_grid_plot(base_dir_1, base_dir_2, key_text, epoch_num=2, bucket=5, name_suffix="", output_dir=""):
    ...
    for i, (key_list_vision, vision_name) in enumerate(vision_evals.items()):
        ...
        make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, epoch_num, bucket, name_suffix)
    
    plt.tight_layout()
    if output_dir[-1] != "/":
        output_dir += "/"
    plt.savefig(f'{output_dir}pretraining_image_ratio_vs_vision_text{text_evals[key_list_text]}_grid_epoch{epoch_num}_bucket{bucket}{name_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_single_plot(base_dir_1, base_dir_2, key_text, key_vision, epoch_num=2, bucket=5, name_suffix="", output_dir=""):
    ...
    if output_dir[-1] != "/":
        output_dir += "/"
    plt.savefig(f'{output_dir}pretraining_image_ratio_vs_vision{vision_evals[key_vision]}_text{text_evals[key_text]}_epoch{epoch_num}_bucket{bucket}{name_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    if args.vision_key == "all":
        create_grid_plot(base_dir_1, base_dir_2, args.text_key, args.epoch_num, args.bucket, args.name_suffix, args.output_dir)
    else:
        create_single_plot(base_dir_1, base_dir_2, args.text_key, args.vision_key, args.epoch_num, args.bucket, args.name_suffix, args.output_dir)
