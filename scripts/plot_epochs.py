...
def make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, name_suffix=""):
    ...
    ax1.plot(sorted_df['epoch'], sorted_df['x1'], 'go-', label=f'Vision {vision_eval}')
    ax2.plot(sorted_df['epoch'], sorted_df['x2'], 'bo-', label=f'Text {text_eval}')
    ...
    plt.title('Finetuning epochs vs Vision and Text Scores')
    plt.savefig(f'finetuning_epochs_vs_vision_{vision_eval}_text_{text_eval}_ratio{ratio}_bucket{bucket}{name_suffix}.png')
    plt.show()
