...
def make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, name_suffix=""):
    ...
    ax1.plot(sorted_df['pretraining_image_ratio'].clip(1e-3), sorted_df['x1'], 'go-', label=f'Vision {vision_eval}')
    ax2.plot(sorted_df['pretraining_image_ratio'].clip(1e-3), sorted_df['x2'], 'bo-', label=f'Text {text_eval}')
    ax1.set_xscale('log')
    ax2.set_xscale('log')

    ax1.set_xlabel('Pretraining Image Ratio')
    ...
    plt.title('Pretraining Image Ratio vs Vision and Text Scores')
    plt.savefig(f'pretraining_image_ratio_vs_vision_{vision_eval}_text_{text_eval}_epoch{epoch_num}_bucket{bucket}{name_suffix}.png')
    plt.show()
