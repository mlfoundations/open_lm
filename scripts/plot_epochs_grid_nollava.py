...
def extract_seed_from_name(dirname):
    """Extracts the seed value from the directory name (assumed format 'seed=<value>')."""
    match = re.search(r'\+x(\d+)', dirname)
    if match:
...
def extract_values(base_dir_1, base_dir_2, key_list_1, key_list_2):
    ...
    for base_name in files_in_base_dir_1:
        try:
            ...
            seed = extract_seed_from_name(base_name)
...
def make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, ratio=0.1, bucket=5, name_suffix="", title=True):
    """
    ...
    data_df['checkpoint_portion'] = data_df['mult'].apply(lambda x: float(x.replace('p', '.')) if x else None)
    ...
    ax.plot(sorted_df['epoch'].clip(1e-3), sorted_df['x1'], 
    ...
    ax2.plot(sorted_df['epoch'].clip(1e-3), sorted_df['x2'], 
             color=text_color, marker='o', linestyle='-', 
             label=f'Text {text_eval}', linewidth=2, markersize=8)
    
    # Make sure the x-axis has 5% in it
    ax.set_xticks(list(sorted_df['epoch'].unique()))


    ax.set_xlabel('Fine-tuning epochs', fontsize=14, fontweight='bold')
...
def create_grid_plot(base_dir_1, base_dir_2, key_text, ratio=2, bucket=5, name_suffix="", output_dir=""):
    ...
    for i, (key_list_vision, vision_name) in enumerate(vision_evals.items()):
        ...
        make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, ax, ratio, bucket, name_suffix)
    
    plt.tight_layout()
    ratio = str(ratio).replace(".", "p")
    if output_dir != "" and output_dir[-1] != "/":
        output_dir += "/"
    plt.savefig(f'{output_dir}finetuning_epochs_datacompdronly_vs_vision_text{text_evals[key_list_text]}_grid_ratio{ratio}_bucket{bucket}{name_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    
def create_single_plot(base_dir_1, base_dir_2, key_text, key_vision, ratio=2, bucket=5, name_suffix="", output_dir=""):
        
    
    fig, ax = plt.subplots(figsize=(6, 3))
    plt.style.use('seaborn-v0_8-pastel')

    make_plot(base_dir_1, base_dir_2, key_vision, key_text, ax, ratio, bucket, name_suffix, title=False)
    
    plt.tight_layout()
    ratio = str(ratio).replace(".", "p")
    if output_dir[-1] != "/":
        output_dir += "/"
    plt.savefig(f'{output_dir}finetuning_epochs_datacompdronly_vs_vision{vision_evals[key_vision]}_text{text_evals[key_text]}_ratio{ratio}_bucket{bucket}{name_suffix}.png', dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bucket", type=int, default=1, help="Bucket to plot.")
    ...
    if args.vision_key == "all":
        create_grid_plot(base_dir_1, base_dir_2, args.text_key, args.ratio, args.bucket, args.name_suffix, args.output_dir)
    else:
        create_single_plot(base_dir_1, base_dir_2, args.text_key, args.vision_key, args.ratio, args.bucket, args.name_suffix, args.output_dir)
