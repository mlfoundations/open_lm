...
def make_plot(base_dir_1, base_dir_2, key_list_vision, key_list_text, name_suffix=""):
    ...
    ax1.plot(sorted_df['bucket'], sorted_df['x1'], 'go-', label=f'Vision {vision_eval}')
    ax2.plot(sorted_df['bucket'], sorted_df['x2'], 'bo-', label=f'Text {text_eval}')

    ax1.set_xlabel('Text pretraining completion ratio')
    ...
    plt.title('Text Pretraining vs Vision and Text Scores')
    plt.savefig(f'text_pretraining_vs_vision_{vision_eval}_text_{text_eval}_ratio{ratio}_epoch{epoch}{name_suffix}.png')
    plt.show()
